    ...
    scene {
        ...
        let pxlMap =
            [
                ...
                let mutable overallOffset = 0
                for x in 0 .. (ctx.width / 2)  do
                    overallOffset, (ctx.width / 2 + x, 0)
                    overallOffset <- overallOffset + 1
                for y in 1.. ctx.height do
                    overallOffset, (ctx.width - 1, y)
                    overallOffset <- overallOffset + 1
                for x in ctx.width - 1 .. -1 .. 0 do
                    overallOffset, (x, ctx.height - 1)
                    overallOffset <- overallOffset + 1
                for y in ctx.height - 1 .. -1 .. 0 do
                    overallOffset, (0, y)
                    overallOffset <- overallOffset + 1
                for x in 1 .. (ctx.width / 2 + 2)  do
                    overallOffset, (x, 0)
                    overallOffset <- overallOffset + 1
            ]
            |> Map.ofList
